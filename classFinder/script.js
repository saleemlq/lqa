...
function getCurrentOrNextClass(text) {
  ...
  if (!schedule.length) return "No valid schedule found.";
  ...
  for (let entry of schedule) {
    ...
    if (now >= start && now < end) {
      return `Current Class: ${entry.student} (${entry.time}, ${entry.day})`;
    }
  }
  ...
  if (upcoming.length) {
    const n = upcoming[0];
    return `Next Class: ${n.student} at ${n.time} on ${n.day}`;
  }
