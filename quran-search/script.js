...
function clearSelections() {
  ...
  document
    .querySelectorAll(".letter-btn, .sign-btn")
...
document
  .getElementById("occurrence")
  .addEventListener("input", () => displayAyahs());

// Prevent propagation
document
  .querySelector(".occurrence-section")
...
function displayAyahs() {
  ...
  filtered.forEach((ayah) => {
    ...
    // Apply current font size immediately
    ...
    let highlightedText = ayah.text
      ...
      .map((ch, i) => {
        ...
        if (
          allLetters.includes(ch) &&
          (allSigns.length === 0 || allSigns.includes(nextCh))
        )
          isHighlighted = true;

        if (
          anyLetters.includes(ch) &&
          (anySigns.length === 0 || anySigns.includes(nextCh))
        )
          isHighlighted = true;
    ...
    div.addEventListener("click", () => {
      window.open(
        `https://quran.com/${ayah.surah}?startingVerse=${ayah.ayah}`,
        "_blank"
      );
    });
...
let currentFontSize = 40; // initial ayah font size in px
...
document.getElementById("font-decrease").addEventListener("click", () => {
  if (currentFontSize > 20) currentFontSize -= 2;
  updateAyahFontSize();
});
